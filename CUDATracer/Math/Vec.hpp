...
namespace Math {
  ...
  template<> struct BinaryOpResultType<float, double> { typedef double type; };
  template<> struct BinaryOpResultType<double, float> { typedef double type; };
  ...
  struct  vec_t<T,1> {
    ...
    inline __both__ vec_t():x(0) {}
    inline __both__ vec_t(const T &x) : x(x) {}
    ...
    inline __both__ explicit vec_t(const vec_t<OT,1> &o) : x(o.x) {}
  ...
  struct  vec_t<T,3> {
    ...
    inline __both__ vec_t() : x(0), y(0), z(0){}
...
#endif
    inline __both__ explicit vec_t(const vec_t<T,4> &v) : x(v.x), y(v.y), z(v.z){}
  ...
  // ------------------------------------------------------------------
  template<typename T>
  struct  vec3a_t : public vec_t<T,3> {
    inline vec3a_t() {}
  ...
  // ------------------------------------------------------------------
  template<typename T>
  struct __align__(16) vec_t<T,4> {
    enum { dims = 4 };
    typedef T scalar_t;
    
    inline __both__ vec_t() : x(0), y(0), z(0), w(0) {}
  ...
  inline __both__ T dot(const vec_t<T,N> &a, const vec_t<T,N> &b)
  {
    T res{0};
    for(unsigned i = 0; i < N; i++) res += a[i]*b[i];
    return res;
  }

  /*! vector cross product */
  template<typename T, unsigned N>
  inline __both__ vec_t<T,N> normalize(const vec_t<T,N> &v)
  {
    return v * 1.f/overloaded::sqrt(dot(v,v));
  }

  /*! vector cross product */
  template<typename T, unsigned N>
  inline __both__ T length(const vec_t<T,N> &v)
  {
    return overloaded::sqrt(dot(v,v));
  }

  template<typename T, unsigned N>
  inline __both__ vec_t<T,N> reflect(const vec_t<T,N>& i, const vec_t<T,N>& n){
    return i - n * dot(n, i) * static_cast<T>(2);
  }

  template<typename T>
  inline __cu_host std::ostream &operator<<(std::ostream &o, const vec_t<T,1> &v)
  {
    o << "(" << v.x << ")";
    return o;
  }
  ...
  template<typename T>
  inline __cu_host std::ostream &operator<<(std::ostream &o, const vec_t<T,2> &v)
  {
    o << "(" << v.x << "," << v.y << ")";
    return o;
  }
  ...
  template<typename T>
  inline __cu_host std::ostream &operator<<(std::ostream &o, const vec_t<T,3> &v)
  {
    o << "(" << v.x << "," << v.y << "," << v.z << ")";
    return o;
  }
  ...
  template<typename T>
  inline __cu_host std::ostream &operator<<(std::ostream &o, const vec_t<T,4> &v)
  {
    o << "(" << v.x << "," << v.y << "," << v.z <<  "," << v.w << ")";
    return o;
  }
...
#include "vec/functors.hpp"
// comparison operators
#include "vec/compare.hpp"
#include "vec/rotate.hpp"
