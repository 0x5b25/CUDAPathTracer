...
namespace CUDATracer{namespace Math {
  ...
  template<typename T>
  inline __both__ typename QuaternionT<T>::Vector RotateAround(
      const typename QuaternionT<T>::Vector& vec,
      const typename QuaternionT<T>::Vector& axis, T rad
  ) {
    const auto quat = QuaternionT<T>::rotate(axis, rad);
    return TransformPoint(quat, vec);
  }
